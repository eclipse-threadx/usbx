...
UINT                            length, length1;
    ...
    _ux_utility_memory_copy(object_info, object, UX_SLAVE_CLASS_PIMA_OBJECT_DATA_LENGTH);
            ...
            if (length1 <= UX_PICTBRIDGE_MAX_FILE_NAME_SIZE)
            {
                ...
                if (length == length1)
                {
                                                    ...
                                                    length) ==  UX_SUCCESS)
                    {
                        ...
                        _ux_utility_event_flags_set(&pictbridge -> ux_pictbridge_event_flags_group, UX_PICTBRIDGE_EVENT_FLAG_DISCOVERY, TX_OR);
