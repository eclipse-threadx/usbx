...
UCHAR                               *device_property_desc_dataset;
    ...
    status = pima -> ux_device_class_pima_device_prop_desc_get(pima, device_property_code, &device_property_desc_dataset, &device_property_desc_dataset_length);
    
    /* Result should always be OK, but to be sure .... */
    if (status != UX_SUCCESS)

        /* We return an error.  */
        _ux_device_class_pima_response_send(pima, UX_DEVICE_CLASS_PIMA_RC_DEVICE_PROP_NOT_SUPPORTED, 0, 0, 0, 0);

    else
    {

        /* Ensure the application's data can fit in the endpoint's data buffer.  */
        if (device_property_desc_dataset_length + UX_DEVICE_CLASS_PIMA_DATA_HEADER_SIZE > UX_SLAVE_REQUEST_DATA_MAX_LENGTH)
        {
