    ...
    {    
        ...
        while (object_length != 0)
        {
            ...
            if (object_length > 0)
                ...
                status =  _ux_device_stack_transfer_request(transfer_request, object_length_received, object_length_received);

            else

                /* It's the last transfer, we need to send a ZLP if the last packet is a full packet.  */
                status =  _ux_device_stack_transfer_request(transfer_request, object_length_received, 0);
